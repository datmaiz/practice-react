...
export const ProductCard: FC<ProductCardProps> = memo(
	({ product, custom, isColorsShown = false, direction = 'vertical' }) => {
		const navigate = useNavigate()

		return (
			<article
				onClick={() => navigate(`/products/${product.productId}`)}
				className={cn(productCardVariants({ direction }), custom)}
			>
				<div
					className={`relative group h-full ${
						direction === 'horizontal' ? 'w-[200px] shrink-0 gap-[32px] h-auto' : ''
					}`}
				>
					<Image
